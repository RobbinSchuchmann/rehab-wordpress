/**
 * Frontend form handler for rehab/assessment-hero.
 *
 ...
 * page-reload-on-submit so the visitor stays in place.
 *
 * Scoped to forms inside .rehab-assessment-hero so it can coexist with the
 * final-cta handler, which binds the same [data-rehab-contact-form] hook.
 */
( () => {
	const init = ( form ) => {
	...
	const boot = () => document.querySelectorAll( '.rehab-assessment-hero form[data-rehab-contact-form]' ).forEach( init );
