...
- **Analytics / marketing tracking — 🟢 code landed (REH-15), inert until enabled on prod.** New mu-plugin `mu-plugins/zz-tracking.php` restores the tracking dropped with **Insert Headers & Footers**. Option-gated *and* host-gated (mirrors `zz-mail-from.php` + `aa-dynamic-host.php`): reads per-site options, and refuses to fire on localhost, the raw dev IP, and `*.cloudwaysapps.com` staging — so dev/staging stay clean even if options leak. Added to **both** deploy allowlists (`deploy.yml` paths + `deploy-to-cloudways.sh` rsync list). Tested: dev/staging inject nothing; a spoofed prod host with options set injects all snippets; malformed IDs are rejected.
  - **What the old dump actually had** (verified, `xscstqwwnp.sql`): a single **GTM-WBCJPZ4** container (the umbrella — GA4 `G-4443JC04QD` fires *through* it; the direct GA4/UA gtag was **commented out**), legacy **UA-163880106-1** (retire), the **lead pixel** (`leads.internetdominators.app`, PxGrabber account **76381674**), and the **Elfsight WhatsApp** widget (app `5edf62f4-6471-425b-948e-327b551420d2`). **Microsoft Clarity was NOT present** — the issue over-listed it; dropped.
  - **GSC verification — already handled.** RankMath already emits `<meta name="google-site-verification" content="googlea63b81b22a9086d8">` (stored in `rank-math-options-general`). That's a *different* token than the old IHAF one (`vnp3yer…`). The mu-plugin supports a `rehab_gsc_verification` option but **leave it unset** unless the client needs that specific old GSC property re-verified (else you double-emit).
...
**Still open for go-live (per scope, after editors start):** ~~REH-15 analytics mu-plugin~~ (DONE, PR #103), REH-16 forms SMTP/recipient, ~~Phase 8 article re-sync~~ (DONE 15 Jul), REH-23 URL normalization, Phase 10 audit against staging, Phase 11 DNS cutover (+ delete the noindex guard, flip `blog_public=1`, re-replace staging URL → `diamondrehabthailand.com`).

**✅ Phase 8 — learning-article re-sync DONE (15 Jul 2026).** Fetched a full fresh live dump (`mysqldump --single-transaction` from the current-live server `68.183.116.65`, app/DB `xscstqwwnp`, 66.6 MB, verified complete). Re-synced **`post_content` only** for the **355 learning articles** on the future-prod server (app `afsjjjgrnm`) via a scoped `UPDATE … JOIN zz_live_posts ON ID` temp-table. **Excluded `clarisse-barrinuevo` (11621)** — the one person page sitting on `template-article.php` (the other 355 are genuine learning articles; verified by slug + title scan). Guardrails: the live site has **0 `template-article.php` pages** (that's our v3 template) so the set is chosen from the **new server's** IDs and pulled from live **by shared post ID**; all 356 exist on live; `[display-map]` legacy shortcode appears on 2 pages but is **pre-existing** on the new server (not introduced). Rehearsed on dev first (editor clean, pages render), then server: 89 MB backup at `~/backup-before-phase8-20260715-042633.sql`, applied (355 rows), temp table dropped, Breeze purged. The freshly-imported live content carries `diamondrehabthailand.com` absolute URLs (353/355) — these get relativised by the **next** step (URL normalization), which is deliberately run **after** the content sync. Related: [[migration-plugin-gaps]].
